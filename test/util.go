...
func CheckProjectMember(projectMember *buddy.ProjectMember, member *buddy.Member, permission *buddy.Permission) error {
	...
	if err := CheckPermission(projectMember.PermissionSet, permission.Name, permission.Description, permission.Id, permission.PipelineAccessLevel, permission.RepositoryAccessLevel, permission.SandboxAccessLevel, permission.ProjectTeamAccessLevel, permission.TargetAccessLevel, permission.EnvironmentAccessLevel); err != nil {
		return err
	}
	return nil
}
...
func CheckPermission(permission *buddy.Permission, name string, desc string, id int, pipelineAccessLevel string, repoAccessLevel string, sandboxAccessLevel string, projectTeamAccessLevel string, targetAccessLevel string, environmentAccessLevel string) error {
	...
	if err := CheckFieldEqualAndSet("Permission.TargetAccessLevel", permission.TargetAccessLevel, targetAccessLevel); err != nil {
		return err
	}
...
func CheckSandboxes(sandboxes *buddy.Sandboxes, count int) error {
	if err := CheckFieldSet("Sandboxes.HtmlUrl", sandboxes.HtmlUrl); err != nil {
		return err
	}
	if err := CheckFieldSet("Sandboxes.Url", sandboxes.Url); err != nil {
		return err
	}
	if err := CheckIntFieldEqual("len(Sandboxes)", len(sandboxes.Sandboxes), count); err != nil {
		return err
	}
	if count > 0 {
		sandbox := sandboxes.Sandboxes[0]
		...
		if err := CheckFieldSet("Sandbox.Id", sandbox.Id); err != nil {
			return err
		}
		if err := CheckFieldSet("Sandbox.Identifier", sandbox.Identifier); err != nil {
			return err
		}
		if err := CheckFieldSet("Sandbox.Name", sandbox.Name); err != nil {
			return err
		}
		if err := CheckFieldSet("Sandbox.Status", sandbox.Status); err != nil {
			return err
		}
	}
	return nil
...
func CheckSandboxEndpoint(endpoint *buddy.SandboxEndpoint, expected *buddy.SandboxEndpoint) error {
	if err := CheckFieldEqualAndSet("Endpoint.Name", *endpoint.Name, *expected.Name); err != nil {
		return err
	}
	if err := CheckFieldEqualAndSet("Endpoint.Endpoint", *endpoint.Endpoint, *expected.Endpoint); err != nil {
		return err
	}
	if err := CheckFieldEqualAndSet("Endpoint.Type", *endpoint.Type, *expected.Type); err != nil {
		return err
	}
	if expected.Region != nil {
		if err := CheckFieldEqualAndSet("Endpoint.Region", *endpoint.Region, *expected.Region); err != nil {
			return err
		}
	} else {
		if err := CheckFieldSet("Endpoint.Region", *endpoint.Region); err != nil {
			return err
		}
	}
	if err := CheckIntFieldSet("Endpoint.Timeout", *endpoint.Timeout); err != nil {
		return err
	}
	return nil
}

func CheckSandbox(sandbox *buddy.Sandbox, expected *buddy.Sandbox, ops *buddy.SandboxOps, expectedStatus string, expectedSetupStatus string, expectedAppStatus string) error {
	id := expected.Id
	name := expected.Name
	identifier := expected.Identifier
	sos := expected.Os
	...
	if id != "" {
		if err := CheckFieldEqualAndSet("Sandbox.Id", sandbox.Id, id); err != nil {
			return err
		} else {
			if err = CheckFieldSet("Sandbox.Id", sandbox.Id); err != nil {
				return err
	...
	if err := CheckFieldEqualAndSet("Sandbox.Name", sandbox.Name, name); err != nil {
		return err
	}
	if identifier != "" {
		if err := CheckFieldEqualAndSet("Sandbox.Identifier", sandbox.Identifier, identifier); err != nil {
			return err
		}
	}
	if expectedStatus != "" {
		if err := CheckFieldEqualAndSet("Sandbox.Status", sandbox.Status, expectedStatus); err != nil {
			return err
		}
	}
	if err := CheckFieldEqualAndSet("Sandbox.Os", sandbox.Os, sos); err != nil {
		return err
	}
	...
	if expectedSetupStatus != "" {
		if err := CheckFieldEqualAndSet("Sandbox.SetupStatus", sandbox.SetupStatus, expectedSetupStatus); err != nil {
			return err
		}
	}
	if err := CheckIntFieldEqualAndSet("len(sandbox.Tags)", len(sandbox.Tags), lenTags); err != nil {
		return err
	}
	if lenTags > 0 {
		if err := CheckFieldEqualAndSet("Sandbox.Tags[0]", sandbox.Tags[0], tags[0]); err != nil {
			return err
		}
	}
	if err := CheckIntFieldEqualAndSet("len(sandbox.Endpoints)", len(sandbox.Endpoints), lenEndpoints); err != nil {
		return err
	}
	if lenEndpoints > 0 {
